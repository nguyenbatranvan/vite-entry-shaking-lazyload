...
var builtinModules = /* @__PURE__ */ getDefaultExportFromCjs(_staticExports);
...
function getCollapsedSourcemap(id, originalCode, originalSourcemap, sourcemapChain, linkMap) {
  ...
  if (originalSourcemap) {
    ...
    const baseSources = sources.map((source2, index) => new Source(resolve(directory, sourceRoot, source2), sourcesContent[index]));
...
function collapseSourcemaps(file, map, modules, bundleSourcemapChain, excludeContent, warn) {
  ...
  if (file) {
    ...
    sources = sources.map((source2) => relative$1(directory, source2));
    file = basename(file);
  }
...
var createHash = () => createHash$1("sha256");
...
var _acorn = /* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  Node,
  Parser,
  Position,
...
var beforeExitEmitter = new EventEmitter();
...
async function loadFsEvents() {
  try {
    ({ default: fsEvents } = await import("./fsevents-MWRBTW63.mjs"));
  } catch (error2) {
...
async function watchInternal(configs, emitter) {
  ...
  await loadFsEvents();
  const { Watcher } = await import("./watch-SSXY2LZA.mjs");
